...
#define MAX_TOKENS          250  // adjust as needed during development (monitor outOfMemory return)
...
typedef struct
{
  u32 net;              // (I) address of the net
  u32 gram;             // (I) address of the grammar
  u32 reserved;         // (I) reserved for use by Sensory. Do not modify.
  ...
  u16 garbageScore;     // (0) comparison score used for calculation of finalScore
  u16 maxPowerEnv;      // (0) measure of energy of utterance
  u16 silLevel;         // (O) measure of silence level before utterance
  u16 duration;         // (0) length in 15 msec frames of utterance
  ...
  s16 netParamA;    // (O) net parameter A
  s16 netParamB;    // (O) net parameter B
  s16 netParamC;    // (O) net parameter C
...
BOOL_S SensoryLPSDFrameReady(appStruct_T *ap);
...
******************************************************************************/
BOOL_S SensoryLPSDProcessFrame(appStruct_T *ap);
